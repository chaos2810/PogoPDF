...
export function inlineText(node: Node, skip = new Set<string>()): string {
...
/**
 * Parse sanitised markdown into a DOM document. One JSDOM window is created for
 * the whole parse (DOMPurify's window, the marked output host, and the returned
 * document all share it) instead of one per stage.
 */
export function sanitizeMarkdown(markdown: string): Document {
  const dom = new JSDOM("");
...
/**
 * Render sanitised markdown blocks into a PDFKit document. The caller owns the
 * document's page/margins/fontSize setup; this walks the block children and
 * emits paragraphs, headings, lists, code blocks, quotes, tables and rules.
 */
export function renderMarkdownBlocks(
  ...
  const hr = () => {
    doc.moveDown(0.5);
    const y = doc.y;
    doc.moveTo(left, y).lineTo(left + contentWidth, y).strokeColor("#999999").stroke();
    doc.moveDown(0.5);
  };
